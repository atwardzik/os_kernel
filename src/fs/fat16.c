...
struct Dentry *FAT16_mount(
        ...
        sb->sb.current_inode_count = 0;
        sb->sb.max_inode_count = 32;
        sb->sb.inode_table = kmalloc(sizeof(void *) * sb->sb.max_inode_count);
        const auto boot_record = read_boot_sector(block_number, hd_op);
        memcpy(&sb->boot_record, &boot_record, sizeof(boot_record));

        struct VFS_Inode *root_inode = FAT16_alloc_inode((struct SuperBlock *) sb);
        root_inode->i_mode |= S_IFDIR;
        root_inode->parent = parent_dir->inode;
